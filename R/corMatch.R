...
function(
  ...
  # NTS CAN WE USE GETCLIP HERE????
  # Work with survey outside template loop
  # If survey is a Wave object, create a file from it
  if(class(survey)=="Wave") {
    fname<-paste0(deparse(substitute(survey)),".wav")
    if(!file.exists(fname)) writeWave(survey,survey<-fname) else warning("Having trouble creating a wav file from ",deparse(substitute(survey)),"--file with name ",fname," already exists.",immediate=TRUE)
  }

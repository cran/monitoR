...
function(
   ...
   if(class(clip)=='character' & class(clip)!='Wave') { 
      ...
      # Read header
      if(file.ext=='wav') {
         header<-readWave(filename=clip,header=TRUE) 
...
#            amp[amp>0]<-0 # Truncate values greater than 0 to 0
        ...
        if(channel=='both') { # Make the right amp matrix 
            wave.R<-channel(wave,which='right')
...
#                amp.R[amp.R>0]<-0 # Truncate values greater than 0 to 0
   ...
   # View a spectrogram only
    if(!interactive) {
       if(class(clip)!='Wave') {
            if(file.ext=='wav') {wave<-channel(readWave(filename=clip,from=start.time,to=start.time+page.length,units='seconds'),which=channel) 
            } else if(file.ext=='mp3') {
                temp.page.l<-page.length+2
                wave<-channel(readMP3(filename=clip,from=round(start.time),to=round(start.time+temp.page.l)),which=channel)
                wave<-cutw(wave=wave,from=0,to=page.length)
#                wave<-channel(readMP3(filename=clip,from=start.time,to=start.time+page.length),which=channel)
            } else wave<-channel(cutw(wave=clip,from=start.time,to=min(start.time+page.length,length(clip@left)/clip@samp.rate)),which=channel)
        } else wave<-channel(cutw(wave=clip,from=start.time,to=min(start.time+page.length,length(clip@left)/clip@samp.rate)),which=channel)
...
#                if(channel=='right' || channel=='both') wave@left<-wave@right[1:(length(wave@right)-0.5*header[['sample.rate']])]
             ...
             wave<-channel(cutw(wave=clip,from=start.time,to=min(start.time+page.length,length(clip@left)/clip@samp.rate)),which=channel)
          }
          t.wave<-length(wave@left)/wave@samp.rate
          if(t.wave<page.length) page.length<-t.wave
          if(is.null(main) && class(clip)=='Wave') {#main<-'Existing wave object'
...
#                }
             ...
             writeWave(object=wave,filename=paste(save.dir,'_',start.time,'-',page.end,'.wav',sep=""))

...
\title{A short introduction to \texttt{monitoR} for acoustic template matching}
\author{Sasha D. Hafner (\texttt{sdh11@cornell.edu}) and Jonathan Katz (\texttt{jonkatz4@gmail.com})}
\begin{document}
\maketitle
...
The motivation behind the development of the monitoR package was for automated detection and identification of animal vocalizations.
For example, the package includes a 23 second recording of some songbirds in a Wave object (defined in the \texttt{tuneR} package) named \texttt{survey}.
...
In this document, we describe how to use \texttt{monitoR} to detect and identify these songs.
...
As \texttt{Wave} objects, all of these recordings can be used directly in the template matching functions.
Users will typically work with wav files, however, as opposed to mp3 files, or \texttt{Wave} objects.
To be consistent with this typical approach, we will save the two clips and the survey as wav files in the working directory.
...
\begin{itemize}
\item For manipulating recordings directly, there are the \texttt{cutw}, \texttt{changeSampRate}, and \texttt{mp3Subsamp} functions.
